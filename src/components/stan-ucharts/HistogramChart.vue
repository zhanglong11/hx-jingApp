...
export default {
  ...
  watch: {
    dataAs: {
      ...
      handler(val) {
        this.render()
        // histogramuCharts[this.canvasId].updateData(val)
      }
    }
  },
  mounted() {
  ...
  methods: {
    render() {
      let defaultOption = {
        ...
        padding: [15, 20, 0, 15], //画布填充边距，顺序为上右下左，同css，但必须4位
          ...
          format: val => {
            ...
            let { type, fixed, name } =
              this.yAxisAs && this.yAxisAs.formatter
                ...
                : defaultSetting
            if (type == 'number') {
              return `${val.toFixed(fixed)}${name}${this.yUnit}`
            } else if (type == 'percent') {
              let newName = name || '%'
              return `${(val * 100).toFixed(fixed)}${name}${newName}${this.yUnit}`
            } else {
              return val.toFixed(0) + `${this.yUnit}`
            }
          },
          min: 0,
          title: '' //Y轴标题
        ...
        //图列配置 legendAs
        legend: {
          show: this.showLegend, //是否显示各类别的图例标识
          position: 'top',
          float: 'left',
          padding: 10,
          margin: 0
