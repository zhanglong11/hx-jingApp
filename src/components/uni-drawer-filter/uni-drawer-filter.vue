...
join
        ...
        <!-- 参与人员 end -->
        <template v-if="['measure', 'qualityAcceptance'].includes(type)">
          <p class="item-line"></p>
          <h6 class="label">批注类型</h6>
          <uni-checkbox-button-group v-model.trim="formData.commentStatus" hasAll join :options="commentStatusList" />
        </template>
      </div>
    </scroll-view>
    <div class="foot">
  ...
  methods: {
    ...
    onChange() {
      let newFormData = {
        ...
        measureStandardIds: this.formData.measureStandardIds.length
          ? this.formData.measureStandardIds.split(',')
          : null,
        commentStatus: this.formData.commentStatus
      }
