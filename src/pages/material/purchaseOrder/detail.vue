        ...
        <block v-if="baseInfo.auditRecordList">
          <uni-card title="审核结果">
            <view v-for="item in baseInfo.auditRecordList" :key="item.auditTime">
              ...
              <!--            </uni-form-item>-->
              <uni-form-item label="审核人" :border="false">{{ item.auditPerson }}</uni-form-item>
          ...
          <uni-form-custom ref="form" :model="form" :rules="rules">
            <uni-card isForm style="margin-top: 10px">
              ...
              <uni-form-item label="审核人">
                <view>{{ baseInfo.currentAuditPerson }}</view>
                <!--                {{ $store.state.user.userName }}-->
                <!--                <uni-select-user v-model="form.auditPersonId" :name.sync="form.auditPerson" placeholder="请选择" />-->
              </uni-form-item>
              ...
              <view v-if="form.auditStatus === 1">
                ...
                <uni-form-item
                  :label="IsContinueValue === 0 ? '采购人' : '下一审核人'"
                  isLink
                  isRequired
                  prop="nextPersonName"
                >
...
export default {
  ...
  methods: {
    ...
    async handleCheck() {
      ...
      await this.fly.material.post(`materialPurchase/audit`, { ...this.form, ...obj }).then(res => {
