<template>
  <div class="main">
    ...
    <uni-card title="检查信息">
      ...
      <uni-form-item v-if="form.planNature <= 4" align="left" label="检查计划名称">
        <view>{{ form.planName }}</view>
      </uni-form-item>
      <uni-form-item v-if="form.planNature <= 4" align="left" label="计划检查日期">
...
<!--      </uni-form-item>-->
    ...
    <template v-if="$hasPower('safetyDetailCommentDetail')">
      <uni-comment
        :id="id"
        ref="comment"
        :canDelete="$hasPower('safetyDetailCommentDelete')"
        type="SafetyInspectRecord"
        @success="refresh"
      />
    </template>
  </div>
...
export default {
  ...
  created() {
    // #ifdef  APP-PLUS
    if (!this.$hasPower('safetyDetailCommentAdd')) {
